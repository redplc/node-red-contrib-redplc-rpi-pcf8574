...
module.exports = function(RED) {
	...
	const sysmodule = syslib.LoadModule("rpi_pcf8574.node");

	const MODE_INPUT  = "0";
	const MODE_OUTPUT = "1";

	RED.nodes.registerType("pcf8574", function (n) {
		var node = this;
		...
		if (node.devadr < 8)
			...
			node.name = "@" + (0x30 + node.devadr).toString(16).toUpperCase();

		node.isdi = (n.mode === MODE_INPUT);
		...
		if (!node.iserror) {
			...
			else if (!sysmodule.initDIO(node.devadr))
